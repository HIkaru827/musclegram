...
export function LikesList({ isOpen, onClose, postId, currentUser, onUserClick }: LikesListProps) {
  const [likedUsers, setLikedUsers] = useState<UserAccount[]>([])
  ...
  const loadLikedUsers = () => {
    try {
      // いいねデータを取得
      const likesData = JSON.parse(localStorage.getItem(`likes_${postId}`) || '[]')
      
      // 全ユーザーを取得
      const allUsers = JSON.parse(localStorage.getItem('musclegram_users') || '[]')
      
      // いいねしたユーザーの詳細情報を取得
      const users = likesData.map((userId: string) => 
        allUsers.find((user: UserAccount) => user.id === userId)
      ).filter(Boolean)
          ...
          <div className="max-h-96 overflow-y-auto space-y-2">
            {isLoading ? (
              ...
              <div className="text-center py-8 text-gray-400">
                ...
                <p className="text-sm">まだいいねがありません</p>
              </div>
            )}
          </div>
        </div>
